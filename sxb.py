...
def wdc_write(ser, data):
    ...
    while True:
        ...
        write_memory(ser, addr, slice)
        offset += size

def wdc_split(ser, data):
    from struct import unpack
    ...
    while True:
        ...
        offset += 6
        slice = data[offset:offset+size]
        with open(f"wdc-0x{addr:04X}.bin", "wb") as f:
            f.write(slice)
        offset += size
...
if __name__ == '__main__':
    ...
    write_p = subparsers.add_parser("wdc", help="Write WDC file to device")
    write_p.add_argument("-e", "--exec")
    write_p.add_argument("file")

    write_p = subparsers.add_parser("split", help="Split WDC file into files")
    write_p.add_argument("file")
